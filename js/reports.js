...
window.reportsSystem = new ReportsSystem();
    // ==================== MÉTODOS CHART.JS COMPLETOS ====================

    initializeBudgetCharts() {
        this.createBudgetRegionalChart();
        this.createBudgetDistributionChart();
    }

    initializeFailureCharts() {
        this.createFailuresByComponentChart();
        this.createFailuresByStatusChart();
        this.createFailuresTrendChart();
        this.createFailuresByVehicleChart();
    }

    initializeExpenseCharts() {
        this.createExpensesByTypeChart();
        this.createExpensesTrendChart();
        this.createExpensesByRegionalChart();
        this.createExpensesByVehicleChart();
    }

    initializeMaintenanceCharts() {
        this.createMaintenanceByVehicleChart();
        this.createMaintenanceByComponentChart();
    }

    initializeVehicleCharts() {
        this.createVehiclesByRegionalChart();
        this.createVehiclesByYearChart();
    }

    initializeExecutiveCharts() {
        this.createKPIRadarChart();
        this.createExecutiveTrendChart();
    }

    // ==================== GRÁFICOS DE PRESUPUESTO ====================

    createBudgetRegionalChart() {
        const ctx = document.getElementById('budgetRegionalChart');
        if (!ctx) return;

        const budgets = this.reportData.budgets;
        const expenses = this.reportData.expenses;

        const regionals = ['Norte', 'Sur', 'Centro'];
        const budgetData = regionals.map(regional => {
            const budget = budgets.find(b => b.regional === regional);
            return budget ? budget.presupuesto : 0;
        });

        const executedData = regionals.map(regional => {
            const regionalExpenses = expenses.filter(e => e.regional === regional);
            return regionalExpenses.reduce((sum, expense) => sum + expense.monto, 0);
        });

        this.charts.budgetRegional = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: regionals,
                datasets: [
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Presupuesto vs Ejecutado por Regional'
                    },
                    legend: {
                        position: 'top',
                    }
    ...
    createBudgetDistributionChart() {
        ...
        budgets.forEach(budget => {
            if (!regionalTotals[budget.regional]) {
                regionalTotals[budget.regional] = 0;
            }
            regionalTotals[budget.regional] += budget.presupuesto;
        });

        const data = Object.values(regionalTotals);
        const labels = Object.keys(regionalTotals);
    ...
    createFailuresByComponentChart() {
        const ctx = document.getElementById('failuresByComponentChart');
        if (!ctx) return;

        const failures = this.reportData.failures;
        const failuresByComponent = this.groupFailuresByComponent(failures);

        const components = Object.keys(failuresByComponent).slice(0, 8);
        const failureCounts = components.map(component => failuresByComponent[component].total);

        this.charts.failuresByComponent = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: components,
                datasets: [{
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Fallas por Componente (Top 8)'
                    }
                },
                scales: {
                    y: {
    ...
    createFailuresByStatusChart() {
        const ctx = document.getElementById('failuresByStatusChart');
        if (!ctx) return;
        ...
        const statusColors = {
            'Abierta': 'rgba(255, 99, 132, 0.8)',
            'En Proceso': 'rgba(255, 205, 86, 0.8)',
            'Resuelta': 'rgba(75, 192, 192, 0.8)'
        };

        this.charts.failuresByStatus = new Chart(ctx, {
            type: 'pie',
            data: {
                labels: Object.keys(failuresByStatus),
                datasets: [{
                    data: Object.values(failuresByStatus),
                    backgroundColor: Object.keys(failuresByStatus).map(status => statusColors[status]),
                    borderColor: Object.keys(failuresByStatus).map(status => statusColors[status].replace('0.8', '1')),
                    borderWidth: 2
                }]
            },
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Distribución de Fallas por Estado'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
    ...
    createFailuresTrendChart() {
        const ctx = document.getElementById('failuresTrendChart');
        if (!ctx) return;
        ...
        const sortedMonths = Object.keys(failuresByMonth).sort();
        const monthLabels = sortedMonths.map(month => {
            const [year, monthNum] = month.split('-');
            return `${monthNum}/${year.substring(2)}`;
        });

        this.charts.failuresTrend = new Chart(ctx, {
            type: 'line',
            data: {
                labels: monthLabels,
                datasets: [{
                    label: 'Fallas por Mes',
                    data: sortedMonths.map(month => failuresByMonth[month]),
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Evolución Mensual de Fallas'
                    }
                },
                scales: {
                    y: {
    ...
    createFailuresByVehicleChart() {
        const ctx = document.getElementById('failuresByVehicleChart');
        if (!ctx) return;
        ...
        this.charts.failuresByVehicle = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: Object.keys(failuresByVehicle),
                datasets: [{
                    label: 'Número de Fallas',
                    data: Object.values(failuresByVehicle),
    ...
    createExpensesByTypeChart() {
        const ctx = document.getElementById('expensesByTypeChart');
        if (!ctx) return;
        ...
        const typeColors = {
            'Mantenimiento Correctivo': 'rgba(255, 99, 132, 0.8)',
            'Combustible': 'rgba(54, 162, 235, 0.8)',
            'Mantenimiento Preventivo': 'rgba(255, 205, 86, 0.8)',
            'Documentación/Seguros': 'rgba(75, 192, 192, 0.8)',
            'Peajes/Viáticos': 'rgba(153, 102, 255, 0.8)',
            'Lavado y Limpieza': 'rgba(255, 159, 64, 0.8)',
            'Repuestos': 'rgba(201, 203, 207, 0.8)',
            'Otros': 'rgba(255, 99, 132, 0.8)'
        };

        this.charts.expensesByType = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: Object.keys(expensesByType),
                datasets: [{
                    data: Object.values(expensesByType).map(data => data.total),
                    backgroundColor: Object.keys(expensesByType).map(type => typeColors[type] || 'rgba(201, 203, 207, 0.8)'),
                    borderColor: Object.keys(expensesByType).map(type => (typeColors[type] || 'rgba(201, 203, 207, 1)').replace('0.8', '1')),
                    borderWidth: 2
                }]
            },
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Distribución de Gastos por Tipo'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
    ...
    createExpensesTrendChart() {
        const ctx = document.getElementById('expensesTrendChart');
        if (!ctx) return;
        ...
        this.charts.expensesTrend = new Chart(ctx, {
            type: 'line',
            data: {
                labels: monthLabels,
                datasets: [{
                    label: 'Gastos Mensuales',
                    data: sortedMonths.map(month => expensesByMonth[month]),
    ...
    createExpensesByRegionalChart() {
        const ctx = document.getElementById('expensesByRegionalChart');
        if (!ctx) return;

        const expenses = this.reportData.expenses;
        const expensesByRegional = this.groupExpensesByRegional(expenses);

        this.charts.expensesByRegional = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: Object.keys(expensesByRegional),
                datasets: [{
                    label: 'Gastos por Regional',
                    data: Object.values(expensesByRegional),
                    backgroundColor: [
                        'rgba(255, 99, 132, 0.8)',
                        'rgba(54, 162, 235, 0.8)',
                        'rgba(255, 205, 86, 0.8)'
    ...
    createExpensesByVehicleChart() {
        const ctx = document.getElementById('expensesByVehicleChart');
        if (!ctx) return;
        ...
        this.charts.expensesByVehicle = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: Object.keys(expensesByVehicle),
                datasets: [{
                    label: 'Gastos por Vehículo',
                    data: Object.values(expensesByVehicle),
                    backgroundColor: 'rgba(153, 102, 255, 0.8)',
                    borderColor: 'rgba(153, 102, 255, 1)',
                    borderWidth: 1
                }]
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Top 10 Vehículos por Gasto'
                    }
                },
                scales: {
                    x: {
    ...
    createMaintenanceByVehicleChart() {
        const ctx = document.getElementById('maintenanceByVehicleChart');
        if (!ctx) return;

        // Datos de ejemplo para mantenimiento
        const maintenanceData = [
            { placa: 'ABC-123', cost: 4500, interventions: 3 },
            { placa: 'XYZ-789', cost: 3200, interventions: 2 },
            { placa: 'DEF-456', cost: 2800, interventions: 2 },
            { placa: 'GHI-789', cost: 1900, interventions: 1 },
            { placa: 'JKL-012', cost: 1500, interventions: 1 }
        ];

        this.charts.maintenanceByVehicle = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: maintenanceData.map(item => item.placa),
                datasets: [{
                    ...
                    data: maintenanceData.map(item => item.interventions),
    ...
    createMaintenanceByComponentChart() {
        const ctx = document.getElementById('maintenanceByComponentChart');
        if (!ctx) return;

        // Datos de ejemplo para componentes
        const componentData = {
            'Bomba de GLP': 1200,
            'Sistema Eléctrico': 800,
            'Tanque': 1500,
            'Válvulas': 600,
            'Sistema RI505': 900,
            'Medidor Másico': 700
        ...
        this.charts.maintenanceByComponent = new Chart(ctx, {
            type: 'polarArea',
            data: {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Costos de Mantenimiento por Componente'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
    ...
    // ==================== GRÁFICOS DE VEHÍCULOS ====================

    createVehiclesByRegionalChart() {
        const ctx = document.getElementById('vehiclesByRegionalChart');
        if (!ctx) return;
        ...
        vehicles.forEach(vehicle => {
            if (!regionalCount[vehicle.regional]) {
                regionalCount[vehicle.regional] = 0;
            }
            regionalCount[vehicle.regional]++;
        });

        this.charts.vehiclesByRegional = new Chart(ctx, {
            type: 'pie',
            data: {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Distribución de Vehículos por Regional'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
    ...
    createVehiclesByYearChart() {
        const ctx = document.getElementById('vehiclesByYearChart');
        if (!ctx) return;
        ...
        vehicles.forEach(vehicle => {
            if (!yearCount[vehicle.año]) {
                yearCount[vehicle.año] = 0;
            }
            yearCount[vehicle.año]++;
        });
        ...
        this.charts.vehiclesByYear = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: sortedYears,
                datasets: [{
                    label: 'Vehículos por Año',
                    data: yearData,
                    backgroundColor: 'rgba(75, 192, 192, 0.8)',
                    borderColor: 'rgba(75, 192, 192, 1)',
                    borderWidth: 1
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Distribución de Vehículos por Año'
                    }
                },
                scales: {
                    y: {
    ...
    createKPIRadarChart() {
        ...
        this.charts.kpiRadar = new Chart(ctx, {
            type: 'radar',
            data: {
                labels: [
                ...
                datasets: [{
                    label: 'KPIs Actuales',
                    data: [
                        kpis.maintenanceEfficiency,
                        kpis.budgetUtilization,
                        100 - kpis.failureRate, // Invertir para que mayor sea mejor
                        Math.max(0, 100 - (kpis.costPerVehicle / 1000)), // Normalizar costo
                        85 // Valor de ejemplo para disponibilidad
                    ],
                    backgroundColor: 'rgba(54, 162, 235, 0.2)',
                    borderColor: 'rgba(54, 162, 235, 1)',
                    borderWidth: 2,
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Análisis de KPIs vs Objetivos'
                    },
                    legend: {
                        position: 'top',
                    }
    ...
    createExecutiveTrendChart() {
        ...
        const failureRates = [12, 15, 8, 10, 7, 9];
        const maintenanceCosts = [4500, 5200, 3800, 4100, 3500, 3900];
        const budgetUtilization = [65, 72, 58, 68, 62, 70];

        this.charts.executiveTrend = new Chart(ctx, {
            type: 'line',
            data: {
                labels: months,
                datasets: [
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Tendencias Clave - Últimos 6 Meses'
                    },
                    legend: {
                        position: 'top',
                    }
    ...
    updateCharts() {
        ...
        if (this.currentReport) {
            this.initializeReportCharts(this.currentReport);
        }
    }
